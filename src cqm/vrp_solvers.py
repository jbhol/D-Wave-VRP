...
from dimod import ConstrainedQuadraticModel, Binary, Integer, quicksum, Real
                    ...
                    for cluster in clusters:
                        ...
                        if len(cluster) > 3:
                            ...
                            if not found:
                                ...
                                # a = {i: Real(f'a_{i}', lower_bound=0) for i in cluster} #represents the arrival times so a_1 means the time arrive at node 1
                                # # # w = {i: Real(f'w_{i}', lower_bound=0) for i in cluster} #represents the wait times so w_1 measn the time waiting at node 1
                                # a['a_leave_depot'] = Real('a_leave_depot', lower_bound=0, upper_bound=0)

                                # max_cost = max(max(row) for row in costs)
                                # scaled_costs = [[cost / max_cost for cost in row] for row in costs]

                                # Define the objective function
                                objective = quicksum(costs[i][j] * x[f'x_{i}_{j}'] for i in cluster for j in cluster if i != j)                           
                                ...
                                for j in cluster:
                                    cqm.add_constraint(
                                        quicksum(x[f'x_{i}_{j}'] for i in cluster if i != j) == 1,
                                ...
                                for i in cluster:
                                    cqm.add_constraint(
                                        quicksum(x[f'x_{i}_{j}'] for j in cluster if i != j) == 1,
                                ...
                                for i in cluster:
                                    if i > 0:
                                        cqm.add_constraint(
                                            quicksum(z[f'z_{i}_{j}'] for j in cluster if i != j) - quicksum(z[f'z_{j}_{i}'] for j in cluster if j != 0 and i != j) == 1,
                                            f'Constraint_3_{i}'
                                        )
                                ...
                                for i in cluster:
                                    for j in cluster:
                                        if i != j and i != 0 and j != 0:  # Exclude the depot (node 0)
                                            ready_time_i = time_intervals[str(i)][0]
                                            due_time_j = time_intervals[str(j)][1]
                                            if ready_time_i + services[0] + costs[i][j] >= due_time_j:
                                                cqm.add_constraint(
                                                    z[f'z_{i}_{j}'] - z[f'z_{j}_{i}'],
                                                    sense=">=",
                                                    rhs=0,
                                                    label=f"ready_time_order_{i}_{j}"
                                                )
                            ...
                            # For double-node clusters, the route is the cluster itself
                            route = cluster
                            optimized_routes.append(copy.deepcopy(route))
                        routes.append(route)
...
class ClarkWright(VRPSolver):
    ...
    def solve(self):
        ...
        # routes = [
        # #             [0,14, 44, 38, 43, 91, 100,0], 
        #             #   [0,43, 14, 38, 44, 91, 100,0]
                    
        #             ]

# [65, 71, 9, 34, 35, 77], [36, 64, 49, 48], [14, 44, 38, 43, 91, 100], [63, 62, 11, 90, 20, 32, 70]

# CLUSTER: [0, 65, 71, 9, 34, 35, 77]
# Optimized Route: [9, 71, 65, 35, 34, 77]
# CLUSTER: [0, 36, 64, 49, 48]
# Optimized Route: [48, 36, 49, 64]
# CLUSTER: [0, 14, 44, 38, 43, 91, 100]
# Optimized Route: [43, 14, 38, 44, 91, 100]
